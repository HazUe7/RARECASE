...
Ensure that all generated values are internally consistent with the following rules (If a rule is violated, adjust the values accordingly): 
(1) if WorstMRSAcute = 6, FinalMRS must be 6 or null; 
(2) if DaysToIT < 30, ITWithin30Days must be 1; 
(3) if EEGDeltaBrush = 1, EEGSlow must be 1; 
(4) if CSFWBCQuant >= 5, CSFPleo must be 1; 
(5) if DaysToIT is assigned a number and/or ItWithin30Days = 1, IT1stLineCombo cannot be "None"; 
(6) if DaysToIT is missing, ITWithin30Days must be 0
"""
...
def extract_case_report(system_prompt, user_prompt, study, text):
    """
    ...
    response = client.chat.completions.create(
        ...
        {"role": "user", "content": user_prompt + " The input publication for data extraction is: " + text}
        ],
        temperature=0
    )
    ...
    output_text = response.choices[0].message.content                                          ### Extract the text output from the model's response
    output_json = output_text.strip()             ### Clean up JSON code block markers
...
def extract_case_series(system_prompt, user_prompt, study, text):
    """
...
def batch_feature_extraction(system_prompt, user_prompt, dataset):
    """
    ...
    for study, text in dataset.items():
        ...
        if n_cases == 1:
            records.append(extract_case_report(system_prompt, user_prompt, study, text))
        else:
            series_records = extract_case_series(system_prompt, user_prompt, study, text)
...
pilot_output = batch_feature_extraction(system_prompt, user_prompt, pilot_data)
print(pilot_output)
...
development_output = batch_feature_extraction(system_prompt, user_prompt, development_data)
